# Plan 3: SupConModule (Stage-1 Pretraining)

**Phase:** 8 — Supervised Contrastive
**Goal:** Create `methods/supcon/` package with `SupConModule` — the stage-1 pretraining module. Wires `SupConLoss`, two-view augmentation, 2-layer projection head, `ClassBalancedSampler` via `SSLDataModule`, and registers `"supcon"` in the dispatcher.
**Req IDs:** SUP-01
**Commit scope:** `methods/supcon/__init__.py`, `methods/supcon/module.py`, `core/dispatcher.py` (import side-effect via `__init__.py`)

---

## Tasks

### Task 3.1: Create `methods/supcon/` package

**File:** `methods/supcon/__init__.py`
**Action:** Create new file.

**Description:**

This file registers `SupConModule` with the method dispatcher on import, matching the pattern in `methods/simclr/__init__.py`. Both `SupConModule` and `SupConFinetuneModule` are registered here (the latter is implemented in Plan 4 but the registration import belongs in the same file).
...
**Verification:** `python -c "import methods.supcon"` — no ImportError.

---

### Task 3.2: Implement `SupConModule` and `SupConFinetuneModule` skeleton
...
Implement `SupConModule` (stage-1 only — NO classifier) following the `SimCLRv1Module` pattern exactly. `SupConFinetuneModule` gets a full skeleton here with a `NotImplementedError` guard in `configure_optimizers` — the full body is filled in Plan 4.
...
cfg_dict = {
    ...
    'max_epochs': 2, 'warmup_epochs': 0, 'batch_size': 32, 'lr': 1e-3,
    'weight_decay': 1e-4, 'optimizer': 'adamw', 'n_views': 2,
    'data_dir': 'data', 'num_workers': 0,
    'supcon': {'temperature': 0.07, 'n_samples_per_class': 2,
               'n_classes_per_batch': 4, 'num_classes': 10, 'projection_dim': 128}
}
cfg = TrainConfig.model_validate(cfg_dict)
m = SupConModule(cfg)
assert not hasattr(m, 'classifier'), 'Stage-1 must NOT have classifier'
print('SupConModule init OK, no classifier: PASS')
mf = SupConFinetuneModule(cfg)
assert hasattr(mf, 'classifier'), 'Stage-2 must have classifier'
print('SupConFinetuneModule init OK, classifier present: PASS')
"
```
...
# Simulate a batch: views [2, B, C, H, W], labels [B]
B = 8
...
print('SupConModule smoke test PASSED.')
"
```
...
- **SC-5**: `method: supcon` is selectable via YAML — `register_method("supcon", SupConModule)` in `__init__.py`.
- **SC-4** (partial): Stage-1 module exists with no classifier, wires `ClassBalancedSampler` via `build_datamodule`.
- **SC-2** (partial): `SupConModule.training_step` passes labels to `SupConLoss`.
