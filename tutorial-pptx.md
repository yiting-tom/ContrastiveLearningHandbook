...
產生 `contrastive_learning_tutorial.pptx`（教學概覽版，10 張投影片）。
...
輸出：`contrastive_learning_tutorial.pptx`

## 投影片結構

| # | 標題 | 設計重點 |
|---|------|----------|
| 1 | 封面 | 深藍背景、薄荷垂直條、青色字幕、技術 tag pills |
| 2 | 什麼是自監督對比學習 | 三欄概念卡片 + 核心思想橫幅 |
| 3 | 專案架構 | Terminal 風格目錄樹 + 資料夾描述卡片 |
| 4 | 四個時代演進 | Era 1–4 四欄卡片，含方法清單與關鍵特色 |
| 5 | 14 種方法總覽 | 完整表格，交替行配色 |
| 6 | 共用訓練框架 | 技術棧卡片 + YAML config 程式碼面板 |
| 7 | 6 種評估工具 | 2×3 卡片網格，各附指令與說明 |
| 8 | 快速開始 | 5 個編號步驟，深色命令列框 |
| 9 | 主要特色 | 三欄特色卡片（可重現性 / 模組化 / 橫向比較） |
| 10 | 結尾 | 深藍背景，大標語 + 需求 pills + 安裝指令 |

## 配色

```
darkBg   : 021B2E   deepBlue : 0A3557   midBlue : 065A82
teal     : 0D9488   cyan     : 06B6D4   mint    : 02C39A
purple   : 7C3AED   amber    : F59E0B   lightBg : EFF6FF
```
